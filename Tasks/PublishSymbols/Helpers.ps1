...
function Add-SourceServerStream {
    ...
    $streamContentFile = New-TemporaryFile
    try {
        ...
        Write-AllText -Path $streamContentFile.FullName -Content $StreamContent
        Write-Verbose "Wrote stream content to: $($streamContentFile.FullName)"
        ...
        try {
            ...
            if ($SymbolsFilePath.Contains(' ')) {
                ...
                $SymbolsFilePath = (New-TemporaryFile).FullName
        ...
        Write-Verbose "Deleting temp source server stream content file: $($streamContentFile.FullName)"
        Remove-Item -LiteralPath $streamContentFile.FullName
    }
}
